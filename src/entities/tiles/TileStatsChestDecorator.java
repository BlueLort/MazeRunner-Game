...
public class TileStatsChestDecorator extends EntityDecorator {

    boolean Opened=false;

    public TileStatsChestDecorator() {
    }

    public TileStatsChestDecorator(Entity decoratedCell) {
        super(decoratedCell);
        this.spritePlaceX=1;
        this.spritePlaceY=0;
    }
    ...
    public void update( double deltaTime) {
       ...
       if( this.decoratedCell.getCollider().isColliding(Controller.getInstance().getPlayer().getCollider())) {
           if(!Opened) {
               Controller.getInstance().getPlayer().OnS_ChestCollision();
               this.Opened = true;
           }
       }
